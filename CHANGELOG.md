...
- **Named User List (`usmt.users[]`)** — New config array for explicitly naming which user accounts to capture (e.g. `["Chris"]` or `["DOMAIN\\Alice", "DOMAIN\\Bob"]`). When non-empty, takes precedence over `userScope`. Each entry becomes a `/ui:` argument; unqualified names are auto-prefixed with `$env:USERDOMAIN`. Displayed in the scan audit and logged during backup.
